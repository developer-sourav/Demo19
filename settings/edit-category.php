...
if(isset($_POST['add-submit']))
{
	...
	if($title!='')
	{
		...
		mysqli_query($connect,"Update `category` SET `title`='".$title."',`parent`='".$parent."' where id=".$id );
	...
	{
		$_SESSION['error_msg']="Category title required";
	}
}
...
?>
					...
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
					  <h5 class="txt-dark">Categories</h5></div>
						...
						?>
                                <form method="post" enctype="multipart/form-data">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" name="title" value="<?php echo $row['title']; ?>" class="form-control" id="inputEmail3" required  placeholder="Title">
                                        </div>
                                    </div>
									<div class="form-group row">
                                        <label for="inputEmail3" class="col-sm-4 col-form-label">Chose Category<font color="red"> (Optional)</font></label>
                                        <div class="col-sm-8">
                                            <select name="parent" class="form-control" id="inputEmail3">
											<option value="">--SELECT PARENT CATEGORY--</option>
											<?php $c=mysqli_query($connect,"SELECT * FROM category WHERE parent=0");
											while($ct=mysqli_fetch_array($c))
											{
											?>
											<option value="<?php echo $ct['id']; ?>"<?php if($row['parent']==$ct['id']) { ?>selected="selected"<?php } ?>><?php echo $ct['title']; ?></option>
											<?php 
											}
											?>
											</select>
